        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-10">
            ...
            <div class="bg-white dark:bg-sand-800 shadow-soft rounded-3xl p-8 border border-sand-200 dark:border-ink-700
                ...
                <p class="text-sm text-ink-600 dark:text-sand-100 mt-1 leading-relaxed">
                    ...
                    Browse, edit, or create new itineraries to plan your next unforgettable trip.
                </p>

                {{--  Smart Insight --}}
                @if($itineraries->count() > 0)
                    <div class="mt-4 text-sm text-ink-500 dark:text-sand-200 italic">
                        @php
                            $next = $itineraries->filter(fn($i) => $i->start_date && \Carbon\Carbon::parse($i->start_date)->isFuture())
                                                ->sortBy('start_date')
                                                ->first();
                        @endphp
                        ...
                        @if($next)
                            ...
                            <span class="font-semibold">{{ \Carbon\Carbon::parse($next->start_date)->format('M j, Y') }}</span>.
                        @else
                    ...
                    <a href="{{ route('traveler.itineraries.index') }}"
                       class="px-4 py-2 rounded-full text-sm font-medium border 
                              {{ !request('filter') 
                                  ...
                                  : 'border-sand-300 dark:border-ink-700 text-ink-700 dark:text-sand-100 hover:bg-sand-100 dark:hover:bg-ink-700' }}">
                        All
                    </a>
                                            ...
                                            <td class="px-4 py-3 text-right whitespace-nowrap">
                                                <div class="flex items-center gap-2 justify-end">
                                                    ...
                                                    <a href="{{ route('traveler.itineraries.show', $itinerary) }}"
                                                       class="px-3 py-1.5 rounded-full border border-ink-500 text-ink-700 dark:text-sand-100 text-sm
