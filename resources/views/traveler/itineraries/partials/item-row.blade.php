            ...
            <form method="POST" action="{{ route('traveler.itineraries.items.store', $itinerary) }}" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div>
                    <input name="type" type="text" placeholder="Type (e.g., Activity)"
                           class="w-full border border-sand-200 dark:border-ink-700 rounded-xl shadow-sm px-3 py-2 text-sm
                                  focus:ring-copper focus:border-copper focus:shadow-glow transition-all duration-200 dark:bg-sand-900" required>
                </div>
                ...
                <div>
                    <input name="start_time" type="datetime-local"
                           class="w-full border border-sand-200 dark:border-ink-700 rounded-xl shadow-sm px-3 py-2 text-sm
                                  focus:ring-copper focus:border-copper focus:shadow-glow transition-all duration-200 dark:bg-sand-900">
                </div>
                ...
                <div class="md:col-span-5 flex justify-end mt-2">
                    <button type="submit"
                            class="px-5 py-2.5 rounded-full bg-gradient-copper text-white font-medium shadow-soft
                                   hover:shadow-glow hover:scale-[1.03] transition-all duration-200 ease-out text-sm">
                        Add Item
                    </button>
                </div>
            </form>
...
{{-- Existing Item Row --}}
<tr x-data="{ open: false, expanded: false }"
    class="group hover:bg-sand-50 dark:hover:bg-sand-900/40 transition-all duration-300 ease-in-out">
    {{-- Type --}}
    <td class="px-4 py-3 whitespace-nowrap text-ink-800 dark:text-ink-100 font-medium">
        {{ ucfirst($item->type) }}
    </td>

    {{-- Title with details preview --}}
    ...
    {{-- Start Time --}}
    <td class="px-4 py-3 whitespace-nowrap text-ink-800 dark:text-ink-100">
        {{ $st ? $st->format('M j, Y g:ia') : '—' }}
    </td>

    {{-- End Time --}}
    <td class="px-4 py-3 whitespace-nowrap text-ink-800 dark:text-ink-100">
        {{ $et ? $et->format('M j, Y g:ia') : '—' }}
    </td>
...
<tr x-show="open" x-transition x-cloak>
    <td colspan="5"
        class="px-6 py-6 bg-sand-50 dark:bg-sand-900/50 border-t border-sand-200 dark:border-ink-700 rounded-b-2xl shadow-inner">
