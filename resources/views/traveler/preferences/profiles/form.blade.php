...
<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('traveler.preference-profiles.index') }}"
               class="group flex items-center gap-2 px-4 py-2 rounded-full border border-copper text-copper 
                      hover:bg-copper hover:text-white hover:shadow-glow hover:scale-[1.03] transition-all duration-200 ease-out font-medium shadow-soft">
    ...
    <div class="py-12 bg-sand min-h-screen" x-data="{ showToast: false }">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            <x-flash-messages />

            <div class="bg-white shadow-soft rounded-3xl p-8 border border-sand-200 hover:shadow-glow hover:scale-[1.005] transition-all duration-200 ease-out">
                <form 
                ...
                >
                    ...
                    {{-- Profile Name --}}
                    <div class="mb-8">
                        <label class="block text-sm font-semibold text-ink-700 mb-2">Profile Name</label>
                        <input type="text" name="name"
                               value="{{ old('name', $preferenceProfile->name ?? '') }}"
                               class="w-full border border-sand-200 rounded-xl shadow-sm focus:ring-copper focus:border-copper
                    ...
                    <div x-data="{ activeTab: 'activities' }" class="space-y-8">
                        ...
                        <div class="flex justify-center mb-6">
                            <nav class="flex flex-wrap justify-center bg-sand rounded-full p-1 shadow-soft">
                                @foreach (['activities' => 'Activities', 'budget' => 'Budget', 'dietary' => 'Dietary', 'accommodation' => 'Accommodation'] as $tab => $label)
                                    <button type="button" 
                                            ...
                                            :class="activeTab === '{{ $tab }}'
                                                ...
                                                : 'text-ink-700 hover:text-copper'"
                                            class="px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-200 ease-out transform hover:scale-[1.03]">
                                        {{ $label }}
                                    </button>
                                @endforeach
                            </nav>
                        ...
                        <div x-show="activeTab === 'activities'" x-transition.opacity.duration.250ms>
                            ...
                            <p class="text-sm text-ink-500 mb-4">Choose your main interests and specific sub-interests below.</p>
                            ...
                            >
                                <div>
                                    <label class="block text-sm font-medium text-ink-700 mb-1">Main Interest</label>
                                    <select x-model="mainId"
                                            class="w-full border border-sand-200 rounded-xl shadow-sm focus:ring-copper focus:border-copper transition">
                                ...
                                {{-- Sub Interests --}}
                                <div>
                                    <label class="block text-sm font-medium text-ink-700 mb-2">Sub-Interests</label>
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        <template x-for="id in selectedSubs" :key="'sel-' + id">
                                            <div class="inline-flex items-center px-3 py-1.5 rounded-full bg-copper-light text-copper-dark text-sm font-medium hover:scale-[1.05] transition-all duration-200 ease-out">
                        ...
                        <div x-show="activeTab === 'budget'" x-transition.opacity.duration.250ms>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                @foreach ([['label'=>'Minimum Budget','name'=>'budget_min','placeholder'=>'$0'], ['label'=>'Maximum Budget','name'=>'budget_max','placeholder'=>'$5000']] as $budget)
                                    <div>
                                        <label class="block text-sm font-medium text-ink-700 mb-1">{{ $budget['label'] }}</label>
                                        <input type="number" name="{{ $budget['name'] }}" min="0"
                                               value="{{ old($budget['name']) ?? ($preferences->where('key', $budget['name'])->first()->value ?? '') }}"
                                               class="w-full border border-sand-200 rounded-xl shadow-sm focus:ring-copper focus:border-copper focus:shadow-glow transition-all duration-200"
                        ...
                        <div x-show="activeTab === 'dietary'" x-transition.opacity.duration.250ms>
                            ...
                            <p class="text-sm text-ink-500 mb-3">Select all dietary preferences that apply.</p>
                            @php
                        ...
                        <div x-show="activeTab === 'accommodation'" x-transition.opacity.duration.250ms>
                            ...
                            <p class="text-sm text-ink-500 mb-3">Select the types of lodging you prefer while traveling.</p>
                            @php
                    ...
                    <div class="mt-10 flex justify-end gap-4">
                        <a href="{{ route('traveler.preference-profiles.index') }}"
                           class="group px-6 py-2.5 rounded-full border border-ink-500 text-ink-700 hover:text-copper hover:border-copper
                                  hover:scale-[1.03] hover:shadow-glow transition-all duration-200 ease-out font-medium shadow-soft">
                            Cancel
                        </a>
                        <button type="submit"
                                :disabled="isSubmitting"
                                class="group flex items-center justify-center gap-2 px-8 py-2.5 rounded-full bg-gradient-copper text-white font-semibold shadow-soft 
                                       hover:shadow-glow hover:scale-[1.03] transition-all duration-200 ease-out disabled:opacity-80 disabled:cursor-not-allowed">
        ...
        <div x-show="showToast"
             ...
             class="fixed bottom-6 right-6 bg-gradient-copper text-white px-5 py-3 rounded-xl shadow-glow text-sm font-medium flex items-center gap-2"
             x-cloak>
